...
private fun LazyListScope.renderTokensTab(
    ...
    onViewAllTokens: () -> Unit // ✅ NEW parameter
) {
    ...
    item {
        ListSectionHeader(
            ...
            sectionTitle = if (searchQuery.isNotBlank()) "Search Results" else "Trending Tokens",
//            actionText = if (searchQuery.isBlank()) "View All" else null, // ✅ Show only when not searching
            onActionClick = if (searchQuery.isBlank()) onViewAllTokens else null, // ✅ Navigate to full list
        )
    }
    ...
    when (displayState) {
        ...
        is LoadingState.Success -> {
            ...
            if (tokens.isEmpty()) {
                ...
                // ✅ IMPORTANT: Already limited to 10 in repository
                // No need to .take(10) here - just render what we got
                items(tokens) { token ->
                    RankedTokenRow(
                        ...
                        logoUrl = token.logoUrl, // ✅ Comes from CoinGecko API
                        fallbackIconColor = getTokenColor(token.symbol),
                        onClick = { onTokenClick(token) }
                    )
                }

                // ✅ Show item count if not searching
                if (searchQuery.isBlank()) {
                    item {
                        Text(
                            "Showing top 10 tokens • View All for more",
...
private fun LazyListScope.renderListsTab(
    ...
    onViewAllDApps: () -> Unit // ✅ NEW parameter
) {
    ...
    item {
        ListSectionHeader(
            searchQuery = searchQuery,
            sectionTitle = if (searchQuery.isNotBlank()) "Search Results" else "Trending DApps",
//            actionText = if (searchQuery.isBlank()) "View All" else null,
            onActionClick = if (searchQuery.isBlank()) onViewAllDApps else null,
        )
    }
    ...
    when (displayState) {
        ...
        is LoadingState.Success -> {
            ...
            if (dappList.isEmpty()) {
                ...
                // ✅ Already limited to 10 in repository
                items(dappList) { dapp ->
                    SiteRow(
                        ...
                        logoUrl = dapp.logoUrl, // ✅ Comes from DeFiLlama API
                ...
                if (searchQuery.isBlank()) {
                    item {
                        Text(
                            "Showing top 10 dApps • View All for more",
                            style = AppTypography.bodySmall,
                            color = AppColors.TextSecondary,
                            modifier = Modifier.padding(vertical = Dimensions.Spacing.small)
                        )
...
private fun LazyListScope.renderPerpsTab(
    ...
    onViewAllPerps: () -> Unit // ✅ NEW parameter
) {
    ...
    item {
        ListSectionHeader(
            searchQuery = searchQuery,
            sectionTitle = if (searchQuery.isNotBlank()) "Search Results" else "Trending Perps",
//            actionText = if (searchQuery.isBlank()) "View All" else null,
            onActionClick = if (searchQuery.isBlank()) onViewAllPerps else null,
        )
    }
            ...
            if (perpList.isEmpty()) {
                ...
                items(perpList) { perp ->
                    PerpRow(
                        ...
                        logoUrl = perp.logoUrl, // ✅ Now populated via PerpLogoProvider
                ...
                if (searchQuery.isBlank()) {
                    item {
                        Text(
                            "Showing top 10 perps • View All for more",
                            style = AppTypography.bodySmall,
                            color = AppColors.TextSecondary,
                            modifier = Modifier.padding(vertical = Dimensions.Spacing.small)
                        )
...
// 4. Update main DiscoverScreen to add navigation callbacks
